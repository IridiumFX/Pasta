...
static double parse_number_literal(const char *start, size_t len, uint8_t *out_fmt) {
    *out_fmt = PASTA_NUM_DEC;
    ...
    /* skip optional leading '-' for prefix detection */
    const char *p = start;
    int neg = 0;
    if (*p == '-') { neg = 1; p++; }
...
static PastaValue *parse_value(Parser *p) {
    ...
    switch (p->current.type) {
        ...
        case TOK_NUMBER: {
            ...
            PastaValue *v = pasta_value_number_fmt(n, fmt);
            if (!v) parser_error(p, "allocation failed");
            advance(p);
            return v;
        }
...
PASTA_API PastaValue *pasta_parse(const char *input, size_t len, PastaResult *result) {
    ...
    if (check(&p, TOK_AT)) {
        ...
        while (!p.had_error && check(&p, TOK_AT)) {
            ...
            /* Section name: label or quoted string (a section name is a
               `label`, so number-/keyword-spelled names like @0 are valid). */
            if (!is_key_token(&p)) {
